...
for pkg in $updates; do
    ...
    if [ -d ".git" ]; then
        git diff HEAD^ HEAD PKGBUILD > "$OUTPUT_DIR/${pkg}_diff.diff"
    else
        cat PKGBUILD > "$OUTPUT_DIR/${pkg}_PKGBUILD_full.diff"
    fi
    
    # 3. Eliminar archivo si está vacío (no hubo cambios reales)
    if [ ! -s "$OUTPUT_DIR/${pkg}_diff.diff" ]; then
        rm -f "$OUTPUT_DIR/${pkg}_diff.diff"
    else
        echo "✅ Cambios encontrados para $pkg -> Guardados en $OUTPUT_DIR/${pkg}_diff.diff"
    fi
done
